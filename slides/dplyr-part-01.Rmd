...
# Load the `big_starwars` dataset

### The `big_starwars` is a subset of the `starwars` data stored in the `dplyr` package:
...
### We'll use a smaller version of this dataset (`big_starwars`) to show `dplyr`'s common data manipluation verbs
...
```{r big_starwars}
library(readr)
big_starwars <- read_csv("https://bit.ly/mini-strwrs")
```
...
knitr::include_graphics(path = "img/rmd-paged-table.png")
```
...
```{r starwars-dataviewer, fig.align='center', out.height='55%', out.width='55%', echo=FALSE}
...
filter(big_starwars, 
        # look for names containing a number
